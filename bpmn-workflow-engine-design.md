...
**Curso:** Fundamentos de Programación y Frameworks Modernos para IA **Entregable:** Diseño e implementación (por grupos) de un motor de workflow inspirado en BPMN 2.0, con comparación frente a Camunda y Activiti. **Naturaleza del entregable:** trabajo grupal. Debe detallarse la contribución individual de cada integrante. **Idioma del modelo de objetos:** el modelo de dominio (clases, atributos, estados) se nombra en **inglés**; la documentación y justificación en español.
...
class TaskStatus(Enum):
    ...
    PENDING \= "PENDING"          \# nunca ejecutada, o reiniciada por incidente (ver was\_reset)
    ...
    TIMED\_OUT \= "TIMED\_OUT"      \# venció un SLA (asignación o completado)
...
class IncidentType(Enum):        \# motivo tipificado de un retorno hacia atrás
    ...
    MISSING\_RESOURCE \= "MISSING\_RESOURCE"

    BUSINESS\_RULE \= "BUSINESS\_RULE"
...
class LogicGate:
    ...
    depends\_on: list\["Task"\] \= field(default\_factory=list)

    expression: Optional\[str\] \= None      \# para COMPLEX / SCRIPT

    endpoint: Optional\[str\] \= None        \# URL REST o ARN Lambda (mock)

    def can\_start(self, target: "TaskInstance", ctx: "WorkflowInstance") \-\> bool:
...
class Transition:                           \# arista tipada del grafo (ver §3.9)
    ...
    max\_retries: Optional\[int\] \= None                       \# tope de rebotes por esta transición

    exhausted\_status: WorkflowStatus \= WorkflowStatus.ERROR \# estado terminal al agotar reintentos

    error\_code: Optional\[str\] \= None                        \# etiqueta específica del error
...
class Task:                                 \# nodo del grafo (definición)
    ...
    targets: list\["Task"\] \= field(default\_factory=list)      \# split (caminos FORWARD)

    incoming: list\["Task"\] \= field(default\_factory=list)     \# dependencias atrás

    backward\_transitions: list\[Transition\] \= field(default\_factory=list)  \# retornos por incidente

    logic\_gate: Optional\[LogicGate\] \= None                   \# join

    required\_resources: list\[ResourceSpec\] \= field(default\_factory=list)

    produced\_resources: list\[ResourceSpec\] \= field(default\_factory=list)

    required\_worker\_type: Optional\[str\] \= None

    task\_type: TaskType \= TaskType.HUMAN

    is\_final: bool \= False

    completion\_policy: CompletionPolicy \= CompletionPolicy.ALL   \# multi-asignación

    quorum: Optional\[int\] \= None                                 \# N si QUORUM

    max\_time\_to\_assign: Optional\[timedelta\] \= None              \# SLA READY \-\> ASSIGNED

    max\_time\_to\_complete: Optional\[timedelta\] \= None            \# SLA ASSIGNED \-\> COMPLETED
...
class Workflow:                             \# plantilla / definición
    ...
    final\_tasks: list\[Task\] \= field(default\_factory=list)

    tasks: list\[Task\] \= field(default\_factory=list)
...
class ResetScope(Enum):
    ...
    ALL\_DOWNSTREAM \= "ALL\_DOWNSTREAM"   \# resetea todo lo alcanzable hacia adelante
...
class Incident:
    ...
    reset\_scope: ResetScope \= ResetScope.ALL\_DOWNSTREAM

    reset\_targets: list\[Task\] \= field(default\_factory=list)
...
stateDiagram-v2
    ...
    ASSIGNED \--\> IN\_PROGRESS: start() / gate OK

    IN\_PROGRESS \--\> COMPLETED: complete() / policy \+ recursos OK

    IN\_PROGRESS \--\> FAILED: fallo

    READY \--\> TIMED\_OUT: SLA de asignación vencido

    ASSIGNED \--\> TIMED\_OUT: SLA de completado vencido

    IN\_PROGRESS \--\> TIMED\_OUT: SLA de completado vencido

    FAILED \--\> PENDING: reintento (transición BACKWARD)

    COMPLETED \--\> PENDING: reset por incidente (wasReset=true)

    IN\_PROGRESS \--\> PENDING: reset por incidente (wasReset=true)
...
stateDiagram-v2
    ...
    PENDING \--\> IN\_PROGRESS: primera tarea lista

    IN\_PROGRESS \--\> SUSPENDED: suspend() (admin)

    SUSPENDED \--\> IN\_PROGRESS: activate() (admin)

    IN\_PROGRESS \--\> COMPLETED: se alcanza una finalTask (happy path)

    IN\_PROGRESS \--\> ERROR: reintentos agotados (exhausted\_status)

    IN\_PROGRESS \--\> CANCELLED: cancel() (admin)
...
4\. D        IN\_PROGRESS it=1

5\. INCIDENT D→C  it=1  type=QUALITY  reason="faltó validar el archivo X"  reset\_scope=ALL\_DOWNSTREAM
...
| Estados de tarea/instancia | Rich lifecycle | `TaskStatus` (READY/ASSIGNED/…/TIMED\_OUT) \+ `WorkflowStatus` (SUSPENDED/ERROR) | ✅ Cubierto |
...
| :---- | :---- | :---- |
| *(nombre)* | *(p. ej. modelo de dominio)* | *(descripción / %)* |
| *(nombre)* | *(orquestador \+ cola)* |  |
| *(nombre)* | *(compuertas lógicas / integraciones mock)* |  |
| *(nombre)* | *(pruebas \+ comparación Camunda/Activiti)* |  |
...
├── PR\_implementation/           \# un archivo por feature/PR

│   ├── PR\_domain\_model.md
